...
并驱动一套回合制事件引擎。数据输入为 `data/v4.6.1.7_card.json`（157 张卡，由
`extract_cards.py` 直接从地图提取，见下）。手抄快照 `data/v20260826_card.json`（153 张）
保留作为提取器的 diff 基线与定价审计的裁判，不再是运行时输入。
...
`v4.6.1.7_card.json` 每张卡带 `source` 字段标注来源。默认只启用**核心种族**
