...
def build_game(
    ...
    # 辅助卡 / 特殊卡、以及提取器标记为「不进卡池」的卡（沙盒测试卡、无法进入卡池的
    # 英雄专属卡、只能由专属效果授予的辅助卡）：保留在卡池对象内（可查询、可被引擎
    # 使用），但不进入可抽取的等级桶。
    _no_draw_sources = set(BASE_EXTRA_SOURCES) | {NO_POOL_SOURCE}
    _no_draw = {
        c.uuid
        for c in pool_cards
        if set(getattr(c, "source", []) or []) & _no_draw_sources
    }
